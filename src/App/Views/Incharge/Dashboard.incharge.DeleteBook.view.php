...
<div class="flex-1 bg-white dark:bg-[#090A0C] ml-16 text-gray-900 dark:text-white h-screen">
    ...
    <main class="container mx-auto px-4 py-8 relative">
        ...
        <div class="flex justify-center items-center min-h-[70vh]">
            <form method="POST" action="/incharge-delete-book" class="max-w-lg bg-gray-50 dark:bg-gray-800 p-6 rounded-lg shadow-md">
                <?=isset($delete_errors) ? loadComponent("ErrorAlert",["errors"=>$delete_errors ?? []]) : ""?>
                <?=isset($success) ? loadComponent("SuccessAlert",["msg" => $success ?? ""]) : ""?>
                <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-300">Delete Book</h2>
                <input type="tel" name="book_no" value='<?=(isset($_GET["book_no"]))?$_GET["book_no"]: ''?>' name="book_no" placeholder="Book ID" class="w-full mb-2 p-2 rounded border dark:border-gray-600 dark:bg-gray-700 dark:text-white"/>
                <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded w-full hover:bg-green-700">Delete</button>
            </form>
        </div>
    </main>
</div>
